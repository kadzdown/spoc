# SPOC: Universal Optical Computer in Stacked Polymers

**Open Concept — Public Domain**  
**@kadzdown** | First Published - November 2025

> **10 ps gates • <1 fJ/bit • 10¹² density • All in light**

---

## 📄 Full Blueprint (4 PDFs)

- [SPOC-v1: Stacked Architecture](SPOC-v1_Main_Paper.pdf)
- [SPOC-v2: Photonic Bit Storage](SPOC-v2_Storage_Supplement.pdf)
- [SPOC-v3: Persistent SSD (GST)](SPOC-v3_Persistent_Storage.pdf)
- [SPOC-v4: Universal Logic + ALU](SPOC-v4_Logic_ALU.pdf)

---

## 🧪 Run the NAND Gate (Meep Sim)
```python
# Coming soon — Colab link
